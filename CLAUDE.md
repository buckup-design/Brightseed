...
| Sidebar toggle rest opacity — **settled Aug 7 2026: stays at 0.** The ~40% proposal measures 1.79:1, not a conformant indicator (only opacity 1 clears 3:1 at 5.75:1), so the middle values buy noise without visibility. 0 holds because the hover target is the whole 240px panel, focus-within covers keyboard, and mobile already renders solid. Reasoning on the `Components/Sidebar` docs page | ✅ Complete |
