...
| ---------- | ------------------- | -------------------------------- | --------------------------------------------------------- |
| Primitives | `--p-*`             | `--p-lime-300`                   | Raw hue values — the palette. Never used in components.   |
| Semantics  | `--ds-*`            | `--ds-color-action-primary`      | Intent-based aliases. Map design intent to primitives.    |
| Components | `--c-{component}-*` | `--c-button-bg`, `--c-badge-surface` | Component-scoped tokens. Every component reads only this tier. |

> **Note:** Existing primitive tokens (`--p-color-forest-700`, `--p-color-lime-300`, etc.) predate the `--p-*` convention and are not being renamed. New primitive tokens use `--p-*`. Component tokens (`--c-*`) are defined per component in that component's stylesheet.
...
5. **Reaching past component tokens** — component code never references semantic or primitive tokens directly. Always use `--c-{component}-*`. If no component token exists for what you need, flag it with `// BRIGHTSEED-TBD: [BLOCKING]` and stop.
