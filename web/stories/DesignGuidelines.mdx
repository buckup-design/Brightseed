...
> **Note:** Primitives are organized by `$type` and all use the `--p-{type}-*` convention (`--p-color-forest-700`, `--p-color-lime-300`, etc.). Component code references `--ds-*` semantics directly; the `--c-*` tier is an optional escape hatch and no `--c-*` tokens exist today.
