...
- **Badge:** the `kind` prop has three values. **Chip** badges are interactive pills (`rounded-full`, hover + focus); **Tag** badges are informational/static (tight 2px radius `--ds-shape-radius-xs`, no hover or focus, never interactive); **Number** badges are count chips. Don't give Chip or Tag the button radius (it reads as a button), and don't wire a click handler onto a Tag.
