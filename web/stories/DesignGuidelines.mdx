...
- **Badge:** Primary badges are pill-shaped (`rounded-full`); secondary badges use the tight 2px radius (`--ds-shape-radius-xs`). Don't give either the button radius (it reads as a button).
...
1. **Brand-poetic names in code:** "Chlorophyll," "Deep Forest," "Garlic Bloom" do not appear in `tokens/`, `bridge/`, components, or documentation.
...
6. **Marketing patterns inside Hummingbird:** hero sections, editorial layouts, large display type belong to the marketing system, not the product. Flag them rather than build them.
