...
Component code references **semantic tokens only** (`--ds-*`) — never primitives directly. The layers exist so that a single design decision (e.g., "primary action should be lime") flows everywhere automatically.
...
| Semantics  | `--ds-*`            | `--ds-color-action-primary`      | Intent-based aliases. Map design intent to primitives. **Component code reads this tier.** |
| Component  | `--c-{component}-*` | `--c-button-bg`                  | Optional escape hatch only — for a local value with no semantic home. None defined yet. |

> **Note:** Primitives are organized by `$type` and all use the `--p-{type}-*` convention (`--p-color-forest-700`, `--p-color-lime-300`, etc.) — the migration is complete; no bare `--color-*` primitives remain. Component code references `--ds-*` semantics directly; the `--c-*` tier is an optional escape hatch and no `--c-*` tokens exist today.
...
| Token                               | Light       | Dark       | Use                                         |
| ----------------------------------- | ----------- | ---------- | ------------------------------------------- |
| `--ds-color-surface-brand`          | forest-900  | sand-900   | Nav bar, header, branded panels             |
| `--ds-color-surface-brand-hover`    | forest-800  | sand-800   | Hovered brand surface                       |
| `--ds-color-surface-brand-active`   | forest-950  | sand-950   | Pressed brand surface                       |
| `--ds-color-surface-brand-subtle`   | forest-50   | sand-950   | Light green tint; accent backgrounds        |
...
| --------------------------------------- | --------------------- | --------------------- | ------------------------------------------ |
| `--ds-color-action-primary`             | lime-300 `#CDE67B`    | lime-300              | Default — theme-invariant                  |
...
| Token                          | Light      | Dark       | Use                                                        |
| ------------------------------ | ---------- | ---------- | ---------------------------------------------------------- |
| `--ds-color-text-default`      | sand-900   | sand-50    | Body copy, primary labels — warm near-black / warm white   |
| `--ds-color-text-default-hover`| sand-950   | sand-50    | One step more pronounced (dark mode hits ceiling at sand-50) |
...
Two distinct components with separate state machines. `link-*` is the inline `<a>` in body copy (always underlined, blue scale). `link-brand` is the `Linktext` button variant (never underlined, lime scale — same family as the primary CTA).
...
| `--ds-color-text-link-brand`      | lime-700   | lime-300   | Linktext default — lime-700 passes AA on white; lime-300 passes AA on sand-950 |
| `--ds-color-text-link-brand-hover`| lime-800   | lime-200   | One step more pronounced |
...
| `--ds-shape-radius-xs`      | 2px    | Tight secondary badges                             |
...
All shadows are tinted with `green-950 (#133019)` for earthy warmth — not neutral gray.
...
How to assemble tokens into the common Forager regions. These map intent → token; each token's appearance is defined in the sections above.
...
Forager-specific constraints on the shared components. Appearance lives in Storybook; these are the rules for using them correctly.
...
- **Badge** — always `rounded-full`; never `rounded` (it reads as a button).
...
1. **Brand-poetic names in code** — "Chlorophyll," "Deep Forest," "Garlic Bloom" do not appear in `tokens/`, `bridge/`, components, or documentation. They live only in `Obsolete/brand-colors-reference.md`.
...
5. **Reaching past the semantic layer** — component code references `--ds-*` semantics only, never primitives (`--p-*`) or raw values directly. If no semantic token exists for what you need, flag it with `// BRIGHTSEED-TBD: [BLOCKING]` and stop.
6. **Marketing patterns inside Forager** — hero sections, editorial layouts, large display type belong to the marketing system, not the product. Flag them rather than build them.
7. **Rounding scientific values for display** — show full precision with units; rounding is a data-layer decision, not a presentation one.
8. **The ambiguous `--primary-*`** — Brightseed splits primary into `--ds-color-surface-brand` (chrome) and `--ds-color-action-primary` (the button). "Primary" alone is forbidden.
9. **Importing from any library beyond shadcn/ui and Recharts** — no MUI, Chakra, Ant, or custom Tailwind plugins.

---

## Domain notes

Forager is a biotech discovery tool; these domain facts shape UI decisions.
