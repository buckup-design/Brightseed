...
Component code references **its own component-scoped semantics only** (`--ds-{component}-*`), never a global `--ds-*`, a primitive, or a raw value directly. Each `--ds-{component}-*` token aliases exactly one global semantic. The layers exist so that a single design decision (e.g., "primary action should be lime") flows everywhere automatically.
...
| Semantics (global) | `--ds-*`    | `--ds-color-action-primary`      | Intent-based aliases. Map design intent to primitives. The only tier that touches primitives. |
| Component (scoped) | `--ds-{component}-*` | `--ds-button-action-primary` | Each component owns a group (labelled `--c-{component}`); every token aliases one global `--ds-*`. **The tier component code reads.** Defined in `tokens/components.css`. |

> **Note:** Primitives are organized by `$type` and all use the `--p-{type}-*` convention (`--p-color-forest-700`, `--p-color-lime-300`, etc.). Component code reads its own component-scoped `--ds-{component}-*` tokens (each aliasing one global `--ds-*`); these live in `tokens/components.css`, grouped under a `--c-{component}` label and generated 1:1 from what each component needs. The `--c-{component}` label names the group, it is not an emitted variable. Dark theme needs no overrides, a token resolves `var(--ds-*)` at point of use, so the `data-theme="dark"` swap flows straight through.
...
- **Reaching past the semantic layer:** component code references `--ds-*` semantics only, never primitives (`--p-*`) or raw values directly. If no semantic token exists for what you need, flag it with `// BRIGHTSEED-TBD: [BLOCKING]` and stop.
