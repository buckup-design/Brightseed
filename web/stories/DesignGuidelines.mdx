...
- **Reaching past the component layer:** component code references its own component-scoped `--ds-{component}-*` tokens only, never a global `--ds-*`, a primitive (`--p-*`), or a raw value directly. If no component token exists for what you need, flag it with `// BRIGHTSEED-TBD: [BLOCKING]` and stop.
