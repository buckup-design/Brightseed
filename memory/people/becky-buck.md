# Becky Buck

**Also known as:** Becky
**Role:** Design lead (pro bono) on Brightseed
**Email:** becky.buck@gmail.com

## Context
Mid-career change. Up-skilling on prototyping with AI, design systems, and AI workflows in general. The Brightseed engagement serves two audiences: (1) Brightseed's engineering team gets a production-ready design system; (2) hiring managers get a case study showing Becky's evolved process with AI tooling.

## How she works
- ADHD/dyslexia-informed — patterns get reflected in the Friday weekly calibration ritual
- Engineering tasks: "just complete the task or tell me exactly what to do"
- Strategic tasks: wants to be challenged, prompted to clarify thinking, write more, build better plans
- GitHub management is Claude's job, not hers — she has stated this as a hard rule
- Linear (Buck-up workspace) is the daily work tracker

## Writing voice
For writing under her name, see `Becky's_tone.md` (referenced in user prefs but not in repo). Voice/writing craft is intentionally dormant right now while she focuses on prototyping.

## Working agreements
- Morning check-in around 8am pulls today's Linear work and asks 3 focused questions
- 4:30pm end-of-day reflection skill closes out the day's focus file
- Friday weekly calibration: how the week went + Linear cycle review
- Use AskUserQuestion tool for clarifying choices, not just inline questions

## Key relationships on Brightseed
Anna (Head of Design consultant), Meng (VP Platform), Chuan (Meng's report). See respective people files.
