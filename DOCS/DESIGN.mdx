...
> **Note:** Existing primitive tokens (`--color-forest-700`, `--color-lime-300`, etc.) predate the `--p-*` convention and are not being renamed. New primitive tokens use `--p-*`. Component tokens (`--c-*`) are defined per component in that component's stylesheet.
