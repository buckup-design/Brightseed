...
1. **`sand-850` for dark cards**: adopt the new `#353430` step (recommended, real gap-filler)?
2. **AA stance**: is "gentle perceived depth" the goal, or do surface boundaries need to clear 3:1 (a bigger change)? This also decides whether the light field border lifts with the dark one.
