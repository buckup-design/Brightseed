...
- **`sand-850` dark cards — APPLIED.** New `--p-color-sand-850` (`#353430`) + a `--ds-color-surface-raised` role. Light is identical to `surface-default` (white), so dark-only by construction. Cards lifted: the `Card` primitive (reports / conversations / projects / report-document / login) and `ResultCard`'s non-predicted body.
  ...
  - **It also took out the ScoreMeter track, found 2026-08-07 and fixed the same day.** The sweep above looks for *inverted* fills; it cannot see two fills that are the *same*. Moving the ResultCard footer to `surface-raised-alt` (dark `sand-800`) put it on the exact value of dark `border-subtle`, which `--c-score-meter-track` aliased — so the track vanished at **1.00:1** and every meter became a green line of arbitrary length, destroying the one channel ScoreMeter exists to deliver (fill *length* means nothing without a visible extent). Measured live, not inferred: track and footer both `#46453f`.
  - **The fix is a translucent track, and the first attempt was wrong in an instructive way.** Repointing to a fixed `sand-900` fixed the footer and immediately broke `Score Meter → On Backdrops`, where a meter sits on `surface-alt` (also `sand-900`) — the collision *moved* rather than went away. A track means "a well in whatever surface I am on", and it sits on at least four; any fixed step collides with one of them. `--ds-color-data-track` is now `black 5%` light / `black 35%` dark, which is a well against every host by construction. Measured: 6.7% Weber on every light surface, 48–56% on every dark one, fill-vs-track ≥ 3:1 across all 21 fill × surface combinations (worst 3.41:1, versus 3.48:1 for the fixed sand-200 it replaces).
  - **Generalizable:** any token whose job is defined *relative to its host* (tracks, wells, scrims, inset shadows) should be an alpha overlay, not a ramp step. A ramp step encodes an absolute position; these need a relative one.
...
2. ~~**Dark intent surfaces on a raised card.**~~ **RESOLVED 2026-08-07 — raised counterparts minted.** Becky's call: do it, for the same reason `surface-raised-alt` exists. Four tokens, `--ds-color-surface-{success,info,warning,critical}-raised`, same intent step and same percentage with the base swapped sand-950 → sand-850.
   - **Confirmed in the browser first**, not just on paper: the "Completed" `StatusBadge` on `Blocks/Reports list view` computed `#242a24` (L 0.0216) on a `#353430` card (L 0.0343) — darker than its host, a dent. After: `#363c33`, lighter than the card at 1.10:1, label still 10.65:1.
   - **Light is unchanged by construction and verified so** — the light recipe is a flat primitive step, not a mix against the page, so `-raised` and base resolve to the identical value. All four pairs measured identical in the running light theme.
   - **Base step only.** `-hover`/`-active` on the intent family are used only by Alert and Sonner, which sit on the canvas or float above it, never inside a card. Add raised hover/active steps only if that changes.
   - Scope was narrower than "four token pairs" implied: `StatusBadge` is the only live on-card consumer.
